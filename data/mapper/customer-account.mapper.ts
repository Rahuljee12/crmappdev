...
import type { EsafCustomerAccountDto } from '@/data/dto/esaf-customer-view.dto';

function trimText(value?: string) {
  return value?.trim() ?? '';
}

function formatBadge(account: EsafCustomerAccountDto) {
...
export function mapEsafCustomerAccountsToDomain(
  ...
  return (accounts ?? [])
    .map((account) => {
      ...
      const badge = formatBadge(account);
